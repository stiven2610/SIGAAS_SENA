...
const get_formatos =async (req, res,next) => {
  try {
    ...
    const { rows } = await pool.query('SELECT a.id_formato_registrado , a.numero_documento_aprendiz,a.fecha_formato,a.numero_documento_instructor_lider,a.codigo_ficha,a.area_actividad,b.id_apartado,b.item_uno,b.item_dos,b.item_tres,b.item_cuatro,c.nombre_completo_aprendiz,c.id_obligacion_mensual,d.nombre_instructor_lider,d.email_instructor_lider,e.nombre_documento FROM formato_registrado AS a, apartado_taller_mensual AS b , aprendiz AS c,instructor_lider AS d, tipo_documento AS e  WHERE c.id_tipo_documento = e.id_tipo_documento and a.numero_documento_aprendiz = c.numero_documento_aprendiz AND a.numero_documento_instructor_lider = d.numero_documento_instructor_lider;');
...
const Get_formato_pdf = async (req, res) => {
  ...
  try {
      const { rows } = await pool.query('select * from formato_registrado where id_formato_registrado = $1', [id_formato_registrado]);
      if (rows.length === 0) {
